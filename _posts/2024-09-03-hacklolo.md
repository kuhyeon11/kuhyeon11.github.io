...
```
The main function essentially provides three options: <br />
Join, login, and exit. <br />
The login status is determined by the value at (user_db + 3352). <br />
...
```
The join function collects user information including ID, password, email, and age. <br />
...
Duplicate IDs are not allowed, and a coupon is issued upon successful registration. <br />
The number of user's infomation is stored in (user_db + 3336). <br />
<br />
...
This function contains two vulnerabilities: <br />
First is OOB. After it receives id and pw, it compares with DB from i=0 to i=32. <br />
Since there is a maximum of 32 user information entries, comparing up to i=32 is incorrect. <br />
The i=32 area contains overall data, so if we manipulate it, we can login with this data. <br />
Second is string comparison. <br />
Generally, when validating a password, we must compare its length. <br />
However, in this system, only the length of the password stored in the DB is compared. <br />
<br />
...
```
There is a game available, and winning it grants regular user status. <br />
...
Although the exact logic is unclear, cycling the last byte of the coupon to the next character allows for up to 4 valid coupons. <br />
...
- After you change direction once, you will be caught by the enemy only if it never comes behind you and only moves to the side until the next time you change direction.
...
### Overwrite main's RA and ROP
- If we change 'Welcome!'(i=32)'s password, admin's password address can be changed.
